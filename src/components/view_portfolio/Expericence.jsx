...
const ExperienceCard = ({ exp, index }) => {
  ...
  return (
    <>
      ...
      <motion.div
        className={`hidden md:flex items-center mb-8 sm:mb-10 ${isLeft ? "flex-row" : "flex-row-reverse"}`}
      ...
      <motion.div
        className="md:hidden flex items-center mb-6"
      ...
      >
        ...
        <div className="relative z-10 flex-shrink-0">
          ...
          />
          <motion.div
            className={`absolute top-1/2 w-6 h-0.5 bg-gradient-to-r ${
              isCurrentJob ? "from-green-500 to-transparent" : "from-purple-500 to-transparent"
        ...
        {/* Experience Card - Always on right */}
        <div className="flex-1 pl-4">
          <motion.div
            className={`bg-gradient-to-br from-[#1a1a2e] to-[#2c2a4a] p-4 rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 border ${
              isCurrentJob ? "border-green-500/40" : "border-purple-700/40"
            }`}
          ...
          >
            <div className="text-left">
              <div className="flex items-center gap-2 mb-2">
                <h3 className="text-lg font-semibold text-purple-300">{exp.position}</h3>
                {isCurrentJob && (
                  <motion.span
                    className="text-xs bg-green-500 text-white px-2 py-0.5 rounded-full"
                    animate={{ scale: [1, 1.05, 1] }}
              ...
              <h4 className="text-base font-medium text-pink-400 mb-2">{`@ ${exp.company}`}</h4>
              <motion.p
                className="text-sm text-purple-200 bg-purple-900/20 px-2 py-1 rounded-full inline-block mb-3"
                whileHover={{ scale: 1.03 }}
              >
                {exp.duration}
              </motion.p>
    ...
    <section className="text-white py-12 sm:py-16 px-4 sm:px-6">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="relative">
          ...
          <div className="hidden md:block absolute left-1/2 transform -translate-x-1/2 w-0.5 bg-gradient-to-b from-purple-500 to-pink-500 h-full rounded-full shadow-md shadow-purple-500/20"></div>
          
          {/* Mobile Timeline - Left */}
          <div className="md:hidden absolute left-2 transform -translate-x-1/2 w-0.5 bg-gradient-to-b from-purple-500 to-pink-500 h-full rounded-full shadow-md shadow-purple-500/20"></div>
